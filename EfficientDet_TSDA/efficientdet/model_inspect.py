...
class ModelInspector(object):
  ...
  def build_model(self, inputs: tf.Tensor) -> List[tf.Tensor]:
    ...
    cls_outputs, box_outputs, dmg_outputs = inference.build_model(
        self.model_name,
        inputs,
        **self.model_config)
    ...
    all_outputs = list(cls_outputs.values()) + list(box_outputs.values()) + list(dmg_outputs.values())
    return all_outputs
  ...
  def freeze_model(self) -> Tuple[Text, Text]:
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      output_dict.update({'damage_predict_%d' % i: outputs[10+i] for i in range(5)})
      signature_def_map = {
          'serving_default':
