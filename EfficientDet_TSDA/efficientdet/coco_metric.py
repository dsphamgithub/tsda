...
class EvaluationMetric():
  ...
  def __init__(self, filename=None, testdev_dir=None, label_map=None, min_threshold=0.05):
  ...
  def evaluate(self, log_level=tf.compat.v1.logging.INFO):
    ...
    if self.testdev_dir:
      ...
      fname = 'detections_testdev_results'
      output_path = os.path.join(self.testdev_dir, fname + '.npy')
      logging.info('Writing output json file to: %s', output_path)
      with tf.io.gfile.GFile(output_path, 'w') as fid:
        np.save(file=fid, arr=np.array(self.detections))    # array in format [image_id, x, y, width, height, score, class]
  ...
  def update_state(self, groundtruth_data, detections):
    ...
    for i, det in enumerate(detections):
      ...
      if not self.filename and not self.testdev_dir:
        # process groundtruth data only if filename is empty and no test_dev.
