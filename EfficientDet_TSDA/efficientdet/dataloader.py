...
class DetectionInputProcessor(InputProcessor):
  ...
  def __init__(self, image, output_size, boxes=None, classes=None, damages=None):
  ...
  def resize_and_crop_boxes(self):
    ...
    damages = tf.gather_nd(self._damages, indices)
    return boxes, classes, damages
...
class InputReader:
  ...
  # TODO: Re-test how/if tf.prints work with this decorator commented out and uncommented, as it was originally
  ...
  def dataset_parser(self, value, example_decoder, anchor_labeler, params):
    ...
    Returns:
      ...
      box_targets_dict: ordered dictionary with keys
        ...
        l-th level.
      dmg_targets_dict: ordered dictionary with keys
        [min_level, min_level+1, ..., max_level]. The values are tensor with
        shape [height_l, width_l, num_anchors * num_classes * num_sectors]. 
        The height_l and width_l represent the dimension 
        of damage logits at l-th level.
    ...
    with tf.name_scope('parser'):
      ...
      damages = data['groundtruth_damages']
      ...
      def transform_damages(dmg, clss):
        """Pads the groundtruth damages tensors to have 0.0 class-wise damage
        for all classes other than the example's true class.
        """
        start_ind = clss[0] - 1
        row_length = dmg.shape[0] * params['num_classes']
        pad_left = tf.cast(start_ind, dtype=tf.int32)
        pad_right = tf.cast(row_length - (start_ind + dmg.shape[0]), tf.int32)
        paddings = tf.convert_to_tensor([[pad_left, pad_right]])
        padded_dmg = tf.pad(dmg, paddings=paddings, mode='CONSTANT', constant_values=0.0)
        return padded_dmg

      if tf.equal(tf.size(damages), 0):  # Skip padding empty damage tensors to prevent errors
        pass
      else:
        damages = tf.map_fn(fn=lambda x: transform_damages(x[0], x[1]), elems=(damages, classes), dtype=tf.float32)
      ...
      if self._is_training:
        ...
        if params['skip_crowd_during_training']:
          ...
          boxes = tf.gather_nd(boxes, indices)
          damages = tf.gather_nd(damages, indices)
      ...
      boxes, classes, damages = input_processor.resize_and_crop_boxes()

      # Assign anchors.
      (cls_targets, box_targets, 
       dmg_targets, num_positives) = anchor_labeler.label_anchors(boxes, classes, damages)
      ...
      classes = pad_to_fixed_size(classes, -1,
                                  [self._max_instances_per_image, 1])
      damages = pad_to_fixed_size(damages, -1, [self._max_instances_per_image, 
                                  params['num_classes'] * params['num_damage_sectors']])
      ...
      return (image, cls_targets, box_targets, dmg_targets, num_positives, source_id,
              image_scale, boxes, is_crowds, areas, classes, damages, image_masks)

  @tf.autograph.experimental.do_not_convert
  def process_example(self, params, batch_size, images, cls_targets,
                      box_targets, dmg_targets, num_positives, source_ids, image_scales,
                      boxes, is_crowds, areas, classes, damages, image_masks):
    ...
    for level in range(params['min_level'], params['max_level'] + 1):
      ...
      labels['dmg_targets_%d' % level] = dmg_targets[level]
      if params['data_format'] == 'channels_first':
        ...
        labels['dmg_targets_%d' % level] = tf.transpose(
            labels['dmg_targets_%d' % level], [0, 3, 1, 2])
    # Concatenate groundtruth annotations to a tensor.
    groundtruth_data = tf.concat([boxes, is_crowds, areas, classes, damages], axis=2)
  ...
  def __call__(self, params, input_context=None, batch_size=None):
    input_anchors = anchors.Anchors(params['min_level'], params['max_level'],
                                    ...
                                    params['anchor_scale'],
                                    params['image_size'])
    anchor_labeler = anchors.AnchorLabeler(input_anchors, params['num_classes'], params['num_damage_sectors'])
