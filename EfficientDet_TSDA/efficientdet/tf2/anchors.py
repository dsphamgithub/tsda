...
class AnchorLabeler(object):
  ...
  def __init__(self, anchors, num_classes, num_sectors, match_threshold=0.5):
    """Constructs anchor labeler to assign labels to anchors.

    Args:
      anchors: an instance of class Anchors.
      num_classes: integer number representing number of classes in the dataset.
      num_sectors: integer number representing number of sectors that ground truth
      bboxes are divided into to calculate damage.
    ...
    unmatched_dmg_target = tf.zeros(num_classes * num_sectors)

    self._target_assigner = target_assigner.TargetAssigner(
        similarity_calc, matcher, box_coder, unmatched_dmg_target)
  ...
  def label_anchors(self, gt_boxes, gt_labels, gt_damages):
    ...
    Args:
      ...
      gt_labels: A integer tensor with shape [N, 1] representing groundtruth
        classes.
      gt_damages: A float tensor with shape [N, num_classes * num_sectors] representing 
      groundtruth sector damages.
    Returns:
      ...
      box_targets_dict: ordered dictionary with keys
        ...
        l-th level.
      damage_targets_dict: dmg_targets_dict: ordered dictionary with keys
        [min_level, min_level+1, ..., max_level]. The values are tensor with
    ...
    cls_targets, _, box_targets, _, dmg_targets, matches = self._target_assigner.assign(
        anchor_box_list, gt_box_list, gt_labels, gt_damages)
    ...
    dmg_targets_dict = self._unpack_labels(dmg_targets)
    num_positives = tf.reduce_sum(
        tf.cast(tf.not_equal(matches.match_results, -1), tf.float32))

    return cls_targets_dict, box_targets_dict, dmg_targets_dict, num_positives
