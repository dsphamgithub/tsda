...
class ClassNet(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      num_sectors: number of sectors over which damage is calculated.
    ...
    self.compute_damages = compute_damages
    ...
    self.damages = self.damages_layer(conv2d_layer, self.num_classes, 
        self.num_anchors, self.num_sectors, name='damage-predict') if compute_damages else None
  ...
  def call(self, inputs, training, **kwargs):
    ...
    for level_id in range(0, self.max_level - self.min_level + 1):
      ...
      if self.damages and not self.feature_only:
        damage_outputs.append(self.damages(image))

    return class_outputs, damage_outputs
  ...
  @classmethod
  def damages_layer(cls, conv2d_layer, num_classes, num_anchors, num_sectors, name):
    """Gets the damages layer in ClassNet class."""
    return conv2d_layer(
        num_classes * num_anchors * num_sectors,
        kernel_size=3,
        bias_initializer=tf.constant_initializer(-np.log((1 - 0.01) / 0.01)),
        padding='same',
        name=name)
...
class DamageNet(ClassNet):
  """A class identical in structure to ClassNet, but that only outputs damage
  predictions.
  """
  def __init__(self, name='damage_net', **kwargs):
    super().__init__(name=name, **kwargs)
...
class EfficientDetNet(tf.keras.Model):
  ...
  def __init__(self,
    ...
    # class/box/damage output prediction network.
    ...
    for head in config.heads:
      if head == 'object_detection':
        self.class_net = ClassNet(
            ...
            compute_damages=(not config.damage_net),
        ...
        # Only use if config.damage_net is True
        self.damage_net = DamageNet(
            num_classes=config.num_classes,
            num_anchors=num_anchors,
            num_sectors=config.num_damage_sectors,
            compute_damages = True,
            num_filters=num_filters,
            min_level=config.min_level,
            max_level=config.max_level,
            is_training_bn=config.is_training_bn,
            act_type=config.act_type,
            repeats=config.box_class_repeats,  # NOTE: May instead use config.damage_repeats
            separable_conv=config.separable_conv,
            survival_prob=config.survival_prob,
            strategy=config.strategy,
            grad_checkpoint=config.grad_checkpoint,
            data_format=config.data_format,
            feature_only=feature_only
        ) if config.damage_net else None
  ...
  def call(self, inputs, training):
    ...
    # call class/box/damage/seg output network.
    outputs = []
    if 'object_detection' in config.heads:
      class_net_out = self.class_net(fpn_feats, training)
      if config.damage_net:
        damage_outputs = self.damage_net(fpn_feats, training)
      else:
        damage_outputs = class_net_out[1]
      
      class_outputs = class_net_out[0]  
      box_outputs = self.box_net(fpn_feats, training)
      outputs.extend([class_outputs, box_outputs, damage_outputs])
...
class EfficientDetModel(EfficientDetNet):
  ...
  def _preprocessing(self,
    ...
    def map_fn(image):
      input_processor = dataloader.DetectionInputProcessor(
          image, image_size)
      # input_processor.normalize_image(mean_rgb, stddev_rgb)   # normalization drastically reduces performance for some reason.
  ...
  def _postprocess(self, cls_outputs, box_outputs, dmg_outputs, scales, mode='global'):
    """Postprocess class, box and damage predictions."""
    if not mode:
      return cls_outputs, box_outputs, dmg_outputs

    if mode == 'global':
      return postprocess.postprocess_global(self.config.as_dict(), cls_outputs,
                                            box_outputs, dmg_outputs, scales)
    if mode == 'per_class':
      return postprocess.postprocess_per_class(self.config.as_dict(),
                                               cls_outputs, box_outputs, dmg_outputs, scales)
    raise ValueError('Unsupported postprocess mode {}'.format(mode))

  def call(self, inputs, image_scales=None, training=False, pre_mode='infer', post_mode='global'):
    """Call this model.
    ...
    if image_scales is None:
      inputs, image_scales = self._preprocessing(inputs, config.image_size,
    ...
    if 'object_detection' in config.heads and post_mode:
      ...
      det_outputs = self._postprocess(outputs[0], outputs[1], outputs[2], image_scales, mode=post_mode)
      outputs = det_outputs + outputs[3:]

    # [nms_boxes, nms_scores, nms_classes, nms_damages, nms_valid_len]
    return outputs
