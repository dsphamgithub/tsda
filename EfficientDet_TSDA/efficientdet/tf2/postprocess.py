...
def merge_class_box_level_outputs(params, cls_outputs: List[T],
                                  box_outputs: List[T], dmg_outputs: List[T]) -> Tuple[T, T, T]:
  """Concatenates class, box and damage of all levels into one tensor."""
  cls_outputs_all, box_outputs_all, dmg_outputs_all  = [], [], []
  ...
  for level in range(0, params['max_level'] - params['min_level'] + 1):
    if params['data_format'] == 'channels_first':
      ...
      dmg_outputs[level] = tf.transpose(dmg_outputs[level], [0, 2, 3, 1])
    cls_outputs_all.append(
        tf.reshape(cls_outputs[level], [batch_size, -1, params['num_classes']]))
    dmg_outputs_all.append(
        tf.reshape(dmg_outputs[level], [batch_size, -1, params['num_classes'] * params['num_damage_sectors']]))
    box_outputs_all.append(tf.reshape(box_outputs[level], [batch_size, -1, 4]))
  return tf.concat(cls_outputs_all, 1), tf.concat(box_outputs_all, 1), tf.concat(dmg_outputs_all, 1)


def topk_class_boxes(params, cls_outputs: T,
                     box_outputs: T, dmg_outputs: T) -> Tuple[T, T, T, T]:
  """Pick the topk class, box and damage outputs."""
  ...
  if max_nms_inputs > 0:
    ...
    dmg_outputs_topk = tf.gather_nd(
        dmg_outputs, tf.expand_dims(indices, 2), batch_dims=1)
  else:
    ...
    dmg_outputs_topk = dmg_outputs

  return cls_outputs_topk, box_outputs_topk, dmg_outputs_topk, classes, indices


def pre_nms(params, cls_outputs, box_outputs, dmg_outputs, topk=True):
  """Detection post processing before nms.

  It takes the multi-level class, box and damage predictions from network, merge them
  ...
  Args:
    ...
    dmg_outputs: a list of tensors for damages, each tensor denotes a level of logits
      with shape [N, H, W, num_class * num_anchors * num_sectors].
    topk: if True, select topk before nms (mainly to speed up nms).

  Returns:
    A tuple of (boxes, scores, classes, damages).
  """
  ...
  cls_outputs, box_outputs, dmg_outputs = merge_class_box_level_outputs(
      params, cls_outputs, box_outputs, dmg_outputs)

  if topk:
    # select topK purely based on scores before NMS, in order to speed up nms.
    cls_outputs, box_outputs, dmg_outputs, classes, indices = topk_class_boxes(
        params, cls_outputs, box_outputs, dmg_outputs)
  ...
  damages = tf.math.sigmoid(dmg_outputs)
  return boxes, scores, classes, damages


def nms(params, boxes: T, scores: T, classes: T, damages: T,
  ...
  Args:
    ...
    damages: a tensor with shape [N, num_classes * num_sectors]
    padded: a bool vallue indicating whether the results are padded.

  Returns:
    A tuple (boxes, scores, classes, damages, valid_lens), where valid_lens is a scalar
  ...
  nms_damages = tf.gather(damages, nms_top_idx)
  nms_classes = tf.cast(
      tf.gather(classes, nms_top_idx) + CLASS_OFFSET, boxes.dtype)
  return nms_boxes, nms_scores, nms_classes, nms_damages, nms_valid_lens
...
def postprocess_global(params, cls_outputs, box_outputs, dmg_outputs, image_scales=None):
  ...
  Returns:
    A tuple of batch level (boxes, scores, classes, damages, valid_len) after nms.
  """
  cls_outputs = to_list(cls_outputs)
  box_outputs = to_list(box_outputs)
  dmg_outputs = to_list(dmg_outputs)
  boxes, scores, classes, damages = pre_nms(params, cls_outputs, box_outputs, dmg_outputs)

  def single_batch_fn(element):
    return nms(params, element[0], element[1], element[2], element[3], True)

  nms_boxes, nms_scores, nms_classes, nms_damages, nms_valid_len = batch_map_fn(
      single_batch_fn, [boxes, scores, classes, damages])
  nms_boxes = clip_boxes(nms_boxes, params['image_size'])
  if image_scales is not None:
    scales = tf.expand_dims(tf.expand_dims(image_scales, -1), -1)
    nms_boxes = nms_boxes * tf.cast(scales, nms_boxes.dtype)
  return nms_boxes, nms_scores, nms_classes, nms_damages, nms_valid_len


def per_class_nms(params, boxes, scores, classes, damages, image_scales=None):
  ...
  Args:
    ...
    damages: A tensor with shape [N, K, num_classes * num_sectors].
    image_scales: scaling factor or the final image and bounding boxes.
  ...
  def single_batch_fn(element):
    ...
    boxes_i, scores_i, classes_i, damages_i = element[0], element[1], element[2], element[3]
    nms_boxes_cls, nms_scores_cls, nms_classes_cls, nms_damages_cls = [], [], [], []
    nms_valid_len_cls = []
    for cid in range(params['num_classes']):
      ...
      damages_cls = tf.gather_nd(damages_i, indices)

      nms_boxes, nms_scores, nms_classes, nms_damages, nms_valid_len = nms(
          params, boxes_cls, scores_cls, classes_cls, damages_cls, False)
      nms_boxes_cls.append(nms_boxes)
      nms_scores_cls.append(nms_scores)
      nms_classes_cls.append(nms_classes)
      nms_damages_cls.append(nms_damages)
    ...
    nms_damages_cls = tf.pad(
      tf.concat(nms_damages_cls, 0), [[0, max_output_size], [0, 0]])
    ...
    return tuple((
        ...
        tf.gather(nms_damages_cls, indices),
...
def postprocess_per_class(params, cls_outputs, box_outputs, dmg_outputs, image_scales=None):
  ...
  box_outputs = to_list(box_outputs)
  dmg_outputs = to_list(dmg_outputs)
  boxes, scores, classes, damages = pre_nms(params, cls_outputs, box_outputs)
  return per_class_nms(params, boxes, scores, classes, damages, image_scales)
