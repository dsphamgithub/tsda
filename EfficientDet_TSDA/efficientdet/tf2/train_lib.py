...
def update_learning_rate_schedule_parameters(params):
  ...
  params['adjusted_learning_rate'] = (params['learning_rate'] * batch_size * hvd.size() / 64)
...
class StepwiseLrSchedule(tf.optimizers.schedules.LearningRateSchedule):
  ...
  def get_config(self):
    return {
        ...
        "first_lr_drop_step": self.first_lr_drop_step,
        "second_lr_drop_step": self.second_lr_drop_step
    }
...
def get_optimizer(params):
  """Get optimizer."""
  update_learning_rate_schedule_parameters(params)
  learning_rate = params['adjusted_learning_rate']
  ...
  if hvd.rank() == 0:
    ...
    if params['model_optimizations'] and 'prune' in params['model_optimizations']:
      ...
      tb_callback = tf.keras.callbacks.TensorBoard(
          log_dir=params['model_dir'] + "/logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"),
          update_freq=params['steps_per_execution'],
          profile_batch=2 if params['profile'] else 0)
...
class EfficientDetNetTrain(efficientdet_keras.EfficientDetNet):
  ...
  def _detection_loss(self, cls_outputs, box_outputs, dmg_outputs, labels, loss_vals):
    ...
    for level in levels:
      ...
      cls_targets_at_level = tf.one_hot(
          ...
          dtype=dtype)
      dmg_targets_at_level = labels['dmg_targets_%d' % (level + self.config.min_level)]

      if self.config.data_format == 'channels_first':
        bs, _, width, height, _ = cls_targets_at_level.get_shape().as_list()
        cls_targets_at_level = tf.reshape(cls_targets_at_level,
                                          [bs, -1, width, height])
        dmg_targets_at_level = tf.reshape(dmg_targets_at_level,
                                          [bs, -1, width, height])
      else:
      ...
      dmg_length = self.config.num_damage_sectors * self.config.num_classes
      dmg_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=dmg_targets_at_level, 
                                                         logits=dmg_outputs[level])
      dmg_loss = dmg_loss / tf.cast(num_positives_sum * dmg_length, dtype)
      if self.config.data_format == 'channels_first':
        dmg_loss = tf.reshape(
            dmg_loss, [bs, -1, width, height, dmg_length])
      else:
        dmg_loss = tf.reshape(
            dmg_loss, [bs, width, height, -1, dmg_length])
      dmg_loss *= tf.cast(
            tf.expand_dims(
                tf.not_equal(
                    labels['cls_targets_%d' % (level + self.config.min_level)],
                    -2), -1), dtype)
      dmg_loss_sum = tf.reduce_sum(dmg_loss)
      dmg_losses.append(tf.cast(dmg_loss_sum, dtype))
  ...
  def train_step(self, data):
    ...
    with tf.GradientTape() as tape:
      if len(self.config.heads) == 2:
        cls_outputs, box_outputs, dmg_outputs, seg_outputs = util_keras.fp16_to_fp32_nested(
            self(images, training=True))
        loss_dtype = cls_outputs[0].dtype
      elif 'object_detection' in self.config.heads:
        cls_outputs, box_outputs, dmg_outputs = util_keras.fp16_to_fp32_nested(
            self(images, training=True))
        loss_dtype = cls_outputs[0].dtype
      ...
      if 'object_detection' in self.config.heads:
        det_loss = self._detection_loss(cls_outputs, box_outputs, dmg_outputs, labels,
                                        loss_vals)
        total_loss += det_loss
...
class EfficientDetNetTrainHub(EfficientDetNetTrain):
  ...
  def __init__(self, config, hub_module_url, name=''):
    ...
    self.damages = efficientdet_keras.ClassNet.damages_layer(
        conv2d_layer,
        config.num_classes,
        num_anchors,
        config.num_damage_sectors,
        name='class_net/damage-predict')
  ...
  def call(self, inputs, training):
    ...
    for i in range(self.config.max_level - self.config.min_level + 1):
      ...
      dmg_outputs.append(self.damages(cls_outputs[i]))
