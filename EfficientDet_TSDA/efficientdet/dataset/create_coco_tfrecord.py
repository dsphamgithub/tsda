...
# ==============================================================================
r"""Convert raw COCO 2017 dataset to TFRecord format with damage labelling.
...
flags.DEFINE_integer('num_damage_sectors', 4, '')
FLAGS = flags.FLAGS
...
def create_tf_example(image,
  ...
  Args:
    ...
    bbox_annotations:
      list of dicts with keys: [u'segmentation', u'area', u'iscrowd',
        u'image_id', u'bbox', u'category_id', u'id', u'sector_damage'] 
        Notice that bounding box
        ...
        size).
        N.B: sector_damage is a list that corresponds to damage ratios of sectors
        in the bounding box in the order [tl, tr, bl, br].
  ...
  if bbox_annotations:
    ...
    for object_annotations in bbox_annotations:
      ...
      # TODO: Generalise for other number of damage sectors
      if 'sector_damage' in object_annotations and FLAGS.num_damage_sectors == 4:
        tl, tr, bl, br = object_annotations['sector_damage']
        tl_dmg.append(float(tl))
        tr_dmg.append(float(tr))
        bl_dmg.append(float(bl))
        br_dmg.append(float(br))
      # TODO: This should probably instead use the damage value that's already in the annotations
      elif 'sector_damage' in object_annotations and FLAGS.num_damage_sectors == 1:
        lst = object_annotations['sector_damage']
        all_ = sum(lst) / len(lst)  # Find average damage
        all_dmg.append(float(all_))
    ...
    if tl_dmg:
      feature_dict.update({
        'image/object/damage/tl':
            tfrecord_util.float_list_feature(tl_dmg),
        'image/object/damage/tr':
            tfrecord_util.float_list_feature(tr_dmg),
        'image/object/damage/bl':
            tfrecord_util.float_list_feature(bl_dmg),
        'image/object/damage/br':
            tfrecord_util.float_list_feature(br_dmg)
      })
    elif all_dmg:
      feature_dict.update({
        'image/object/damage/all':
            tfrecord_util.float_list_feature(all_dmg)
      })
    if include_masks:
...
def _create_tf_record_from_coco_annotations(image_info_file,
  ...
  def _get_caption_annotation(image_id):
    if img_to_caption_annotation and 'caption' in img_to_caption_annotation:
