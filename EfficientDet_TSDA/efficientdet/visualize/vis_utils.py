...
def draw_bounding_box_on_image(image,
  ...
  # Each display_str has a top and bottom margin of 0.00x.
  total_display_str_height = (1 + 2 * 0.00) * sum(display_str_heights)
  ...
  for display_str in display_str_list[::-1]:
    ...
    margin = np.ceil(0.00 * text_height)
  ...
  for i in range(boxes.shape[0]):
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        dmg_str_1 = '{:.2f} {:.2f}'.format(damages[i][0], damages[i][1])  # tl, tr
        dmg_str_2 = '{:.2f} {:.2f}'.format(damages[i][2], damages[i][3])  # bl, br
        box_to_display_str_map[box].append(dmg_str_1)
        box_to_display_str_map[box].append(dmg_str_2)
        if agnostic_mode:
