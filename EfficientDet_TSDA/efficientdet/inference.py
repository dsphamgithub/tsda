...
def build_model(model_name: Text, inputs: tf.Tensor, **kwargs):
  ...
  if kwargs.get('use_keras_model', None):
    ...
    def model_arch(feats, model_name=None, **kwargs):
      ...
      cls_out_list, box_out_list, dmg_out_list = model(feats, training=False)
      ...
      assert len(dmg_out_list) == config.max_level - config.min_level + 1
      cls_outputs, box_outputs, dmg_outputs = {}, {}, {}
      for i in range(config.min_level, config.max_level + 1):
        ...
        dmg_outputs[i] = dmg_out_list[i - config.min_level]
      return cls_outputs, box_outputs, dmg_outputs

  else:
    model_arch = det_model_fn.get_model_arch(model_name)

  cls_outputs, box_outputs, dmg_outputs = utils.build_model_with_precision(
  ...
  if mixed_precision:
    ...
    dmg_outputs = {k: tf.cast(v, tf.float32) for k, v in dmg_outputs.items()}
...
def det_post_process(params: Dict[Any, Any], cls_outputs: Dict[int, tf.Tensor],
                     ...
                     dmg_outputs: Dict[int, tf.Tensor], scales: List[float]):
  ...
  if params.get('combined_nms', None):
    ...
    nms_boxes, nms_scores, nms_classes, nms_damages, _ = postprocess.postprocess_combined(
        params, cls_outputs, box_outputs, dmg_outputs, scales)
  else:
    nms_boxes, nms_scores, nms_classes, nms_damages, _ = postprocess.postprocess_global(
        params, cls_outputs, box_outputs, dmg_outputs, scales)
  ...
  detections = [
      ...
      nms_scores,
      nms_classes,
      # FIXME: Temporary solution, need to make it flexible to different num_damage_sectors values
      nms_damages[:, :, 0],
      nms_damages[:, :, 1],
      nms_damages[:, :, 2],
      nms_damages[:, :, 3]
...
def visualize_image_prediction(image,
  ...
  scores = prediction[:, 5]
  damages = prediction[:, 7:11]

  return visualize_image(image, boxes, classes, scores, damages, label_map, **kwargs)
...
class ServingDriver(object):
  ...
  def build(self, params_override=None):
    ...
    with self.sess.graph.as_default():
      ...
      class_outputs, box_outputs, damage_outputs = build_model(self.model_name, images,
                                                               **params)
      params.update(dict(batch_size=self.batch_size))
      detections = det_post_process(params, class_outputs, box_outputs, damage_outputs, scales)
...
class InferenceDriver(object):
  ...
  def inference(self, image_path_pattern: Text, output_dir: Text, **kwargs):
    ...
    with tf.Session() as sess:
      ...
      detections_batch = det_post_process(params, class_outputs, box_outputs, damage_outputs,
                                          scales)
